function [bit_stream_out1] = ofdm_demod(EQ_mode, recived_signal, P, L, series_msg_length, mask, zeros_to_append, M, imageSize, bitsPerPixel, colorMap, trainingblock, TB_size, data_size, mu, a,plot_error)
...
%     subcarriers_bitloading = zeros(rows_sent,temp_c);
    ...
    error_matrix_bitloading = zeros(sum(mask)/2,temp_c);
    ...
    for m=1:c1
        for i=2:P+1
            ...
            if mask(i)
                ...
                if (EQ_mode == 1) && (plot_error==1)
                    error_matrix_bitloading(k,m) = error_matrix(i,m);
                end
                k = k +1;
            end
        end
        ...
        if EQ_mode == 0
            subplot(2,2,1); plot(abs(H_lsr(:,z))); 
            title('Estimated Channel H (Packet-based LS)');
            xlabel('Frequency [Hz]');
            ylabel('Magnitude');
            drawnow;

            subplot(2,2,3); plot(ifft(H_lsr(:,z))); 
            title('Estimated Channel h (Packet-based LS)');
            xlabel('Frequency [Hz]');
            ylabel('Magnitude');
            drawnow;
            ...
            subplot(2,2,1); plot(abs(H_lms_matrix(:,m))); 
            title('Estimated Channel H (Adaptive-LMS)');
            xlabel('Frequency [Hz]');
            ylabel('Magnitude');
            drawnow;
            if plot_error==1
                subplot(2,2,4); plot(error_matrix_bitloading(:,1:m)'); 
                title('error in frequency bins');
                xlabel('frame');
                ylabel('Magnitude');
                drawnow;
            end
            subplot(2,2,3); plot(ifft(H_lms_matrix(:,m))); 
            title('Estimated Channel h (Adaptive-LMS)');
            xlabel('Frequency [Hz]');
            ylabel('Magnitude');
            drawnow;
